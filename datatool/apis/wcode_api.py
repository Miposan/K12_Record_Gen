...
class WCodeAPI(BaseAPI):   
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.api_key = os.getenv("WCODE_API_KEY", None)
        if self.api_key is None:
            raise ValueError("WCODE_API_KEY is not set")
        self.client = OpenAI(api_key=self.api_key, base_url="https://wcode.net/api/gpt/v1/chat/completions")
