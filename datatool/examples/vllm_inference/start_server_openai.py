# python examples/test_glm4o/start_server_openai.py --config-path configs/9b-video-aimv2-mrope-sandwich-adapt-position-reasoning-t-patch --model-weight-path /workspace/official_pretrains/megatron_home/instruct-glm4v-9b--tpatch2-20250529/iter_0001200 --tensor-parallel-size 2 --t-patch-size 2 --gpu-memory-utilization 0.8


# python examples/test_glm4o/start_server_openai.py --config-path /opt/glm-vllm/configs/9b-video-aimv2-mrope-sandwich-adapt-position-reasoning --model-weight-path /workspace/wg/glm-train-dev/ckpt/doubaov2-fix-v8t1-bs128-roll1train1-expand2-clip-higner/iter_0000175 --tensor-parallel-size 1 --t-patch-size 1 --gpu-memory-utilization 0.8


 # python start_server_openai.py --config-path /opt/glm-vllm/configs/9b-video-aimv2-mrope-sandwich-adapt-position-reasoning-t-patch --model-weight-path /workspace/official_pretrains/megatron_home/instruct-glm4v-9b--tpatch2-20250529/iter_0001200 --tensor-parallel-size 2 --t-patch-size 2 --gpu-memory-utilization 0.8
...
def predict(**kwargs):
    ...
    try:
        ...
        if data.get("n", 1) > 1:
            return [choice['message']['content'] for choice in response.json()['choices']], 200
        else:
